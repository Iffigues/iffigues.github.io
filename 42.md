...
<script>
 ...
 const state = pdfStates[key];
  state.pdfDoc.getPage(state.pageNum).then(page => {
    ...
    // Calcule le scale en fonction de la largeur du parent
    const containerWidth = canvas.parentElement.clientWidth;
    const unscaledViewport = page.getViewport({ scale: 1 });
    const dynamicScale = (containerWidth - 20) / unscaledViewport.width; 
    
    // On utilise le plus petit entre 1.5 et le scale dynamique pour éviter le flou
    const finalScale = Math.min(1.5, dynamicScale);
